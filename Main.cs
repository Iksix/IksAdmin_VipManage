...
public class VipModel
{
    ...
    {
        get => Expires < DateTimeOffset.UtcNow.ToUnixTimeSeconds();
    }
...
public class Main : AdminModule
{
    ...
    public override void Ready()
    {
        ...
        Api.RegisterPermission("other.vip_manage", Config.AdminPermissionFlags);
    }
    ...
    public override void OnAllPluginsLoaded(bool hotReload)
    {
        ...
        try
        {
            ...
            _dbString = VipApi!.GetDatabaseConnectionString;
        }
        catch (Exception _)
        {
    ...
    public HookResult OnPlayerConnect(EventPlayerConnectFull @event, GameEventInfo info)
    {
        ...
        Task.Run(async () => { _playerVipGroup[slot] = await GetPlayerVipGroup(accountId); });
    ...
    private HookResult OnMenuOpenPre(CCSPlayerController player, IDynamicMenu menu, IMenu gameMenu)
    {
        ...
        menu.AddMenuOption("vip_manage", Localizer["MenuOption.VipManage"], (_, _) => { OpenSelectPlayerMenu(player, menu); },
    ...
    private void OpenVipManageMenu(CCSPlayerController caller, CCSPlayerController target, IDynamicMenu? backMenu = null!)
    {
        ...
        menu.AddMenuOption("group:" + group.Group, Localizer["MenuOption.Group"].AReplace(["group"], [group.Group]), (_, _) =>
        {
            ...
            {
                mn.BackAction = caller => { OpenVipManageMenu(caller, target, backMenu); };
                _playerVipGroup[target.Slot].Group = g!;
                OpenVipManageMenu(caller, target, backMenu);
                Task.Run(async () =>
                {
                    await UpdateVip(accountId, group);
                    Server.NextFrame(() => { UpdateVipOnServer(target); });
                });
            });
        });

        menu.AddMenuOption("expires:" + group.Expires,
            Localizer["MenuOption.Expires"].AReplace(["date"], [group.Expires != -1 ? Utils.GetDateString((int)group.Expires) : "NONE"]), (_, _) =>
            {
    ...
    private void OpenVipAddMenu(CCSPlayerController caller, CCSPlayerController target, IDynamicMenu? backMenu = null!)
    {
        ...
        menu.AddMenuOption("group:" + group.Group, Localizer["MenuOption.Group"].AReplace(["group"], [group.Group]), (_, _) =>
        {
            MenuUtils.SelectItemDefault<string?>(caller, "vm_group", groups.ToList()!, (g, mn) =>
            {
                mn.BackAction = caller => { OpenVipAddMenu(caller, target, backMenu); };
                _playerVipGroup[target.Slot].Group = g!;
